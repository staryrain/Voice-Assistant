...
class Config:
    def __init__(self):
        ...
        self.auth = {
            "app_key": "2078616776",
            "access_key": "7KpKwBDOlRBlEXB6sbcIyhJa-01pUa-U"
        }
...
class AsrWsClient:
    ...
    async def read_audio_data(self, file_path: str) -> bytes:
        try:
            ...
            # 读取wav信息
            channel_num, samp_width, frame_rate, frame_count, pcm_data = CommonUtils.read_wav_info(content)

            logger.info(
                f"Original WAV - channels:{channel_num}, "
                f"sample_width:{samp_width}, "
                f"sample_rate:{frame_rate}, "
                f"frames:{frame_count}"
            )

            # 如果不是标准16kHz 16bit mono → 重新转
            if frame_rate != 16000 or samp_width != 2 or channel_num != 1:
                logger.info("Resampling audio to 16kHz 16bit mono...")
                content = CommonUtils.convert_wav_with_path(file_path, 16000)
                channel_num, samp_width, frame_rate, frame_count, pcm_data = CommonUtils.read_wav_info(content)

                logger.info(
                    f"Resampled WAV - channels:{channel_num}, "
                    f"sample_width:{samp_width}, "
                    f"sample_rate:{frame_rate}, "
                    f"frames:{frame_count}"
                )

            return pcm_data  # 返回裸PCM
    ...
    def get_segment_size(self, content: bytes) -> int:
        ...
        segment_size = bytes_per_second * self.segment_duration // 1000
        return segment_size
